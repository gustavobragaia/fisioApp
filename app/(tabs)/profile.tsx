...
const EditNameModal = ({ visible, onClose, onSave, initialName }: { 
  visible: boolean; 
  onClose: () => void; 
    ...
    >
      <View className="flex-1 justify-center items-center bg-black/50">
        <View className="w-5/6 bg-white p-6 rounded-xl shadow-lg">
          <Text className="text-lg font-bold text-deepBlue mb-4">Alterar Nome</Text>
...
export default function ProfileScreen() {
  ...
  const handleUpdateName = async (newName: string) => {
    ...
    try {
      ...
      const { data, error } = await updateUserProfile(user.id, { name: newName });
      ...
      <View className="flex-1 mt-4">
        ...
        >
          <View className="flex-1">
            ...
            <Text className="text-gray-500 text-sm mt-1">{userProfile?.name || 'Não definido'}</Text>
          </View>
          {isSaving && <ActivityIndicator size="small" color={colors.light.deepBlue} />}
        </TouchableOpacity>
        ...
        {userProfile?.email && (
          <View className="flex-row items-center p-4 bg-white rounded-lg mb-3 shadow-sm">
            <View className="flex-1">
              ...
              <Text className="text-gray-500 text-sm mt-1">{userProfile.email}</Text>
            </View>
          </View>
        )}
        ...
        {userProfile?.cpf && (
          <View className="flex-row items-center p-4 bg-white rounded-lg mb-3 shadow-sm">
            <View className="flex-1">
              ...
              <Text className="text-gray-500 text-sm mt-1">{userProfile.cpf}</Text>
            </View>
          </View>
        )}
        ...
        {userProfile?.empresa && (
          <View className="flex-row items-center p-4 bg-white rounded-lg mb-3 shadow-sm">
            <View className="flex-1">
              ...
              <Text className="text-gray-500 text-sm mt-1">{userProfile.empresa}</Text>
            </View>
          </View>
        )}
    ...
    <SafeAreaView className="flex-1 bg-paleSand">
      <ScrollView className="flex-1 px-4">
        <View className="items-center pt-6 pb-4">
          <ProfileAvatar name={userProfile?.name || 'Usuário'} size={120} />
          <Text className="text-textPrimary text-xl font-bold mt-4">{userProfile?.name || 'Usuário'}</Text>
        </View>

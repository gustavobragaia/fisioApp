...
export const generatePainExerciseRecommendations = async (
  ...
  try {
    let exerciseQuery = supabase
      .from("exercises")
      .select("*")
      .eq("region_name", painLocation)

    console.log(`Finding Exercise for pain location: ${painLocation})}`);
    console.log("Exercise query:", await exerciseQuery);

    const isPainHigh = isPainLevelHigh(painLevel);

    if (isPainHigh) {
      exerciseQuery = exerciseQuery
        .in("group_type", ["Alívio imediato da dor", "Alongamento"])
...
export const generateMentalHealthExerciseRecommendations = async (
  ...
  try {
    let exerciseQuery = supabase
      .from("exercises")
      .select("*")
      .eq("region_name", mentalState);

    console.log(`Finding Exercise for mental state: ${mentalState}`);
    console.log("Exercise query:", await exerciseQuery);

    const isImpactHigh = isHighImpactLevel(impactLevel);
    ...
    const selectedExerciseIds = exerciseIds.slice(0, 5);
...
const mapPainLocationToRegion = (painLocation: string): string => {
  ...
  return String(
    painLocation.charAt(0).toLocaleUpperCase() + painLocation.slice(1)
  );
};
...
const isHighImpactLevel = (impactLevel: string): boolean => {
  ...
  return highImpactKeywords.some((keyword) =>
    lowerImpactLevel.includes(keyword)
  );
};

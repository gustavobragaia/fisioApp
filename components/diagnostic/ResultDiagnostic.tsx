...
export function ResultDiagnostic({
  ...
  const parseApiResponse = (): ApiResponseData | null => {
    try {
      ...
      const response = typeof params.apiResponse === 'string'
    ...
    <ScrollView className="flex-1">
      ...
      <View className="items-center mt-12">
        ...
        <Text className="text-xl text-textPrimary mt-2 text-center">
          {type === 'pain'
            ? `${user?.user_metadata?.name || user?.email || 'Usuário'}, separei esse treino personalizado para você!`
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <EmptyState
            title={`Nenhum ${type === 'pain' ? 'exercício' : 'prática'} encontrado`}
            variant="sad"
          />
        }
      />
